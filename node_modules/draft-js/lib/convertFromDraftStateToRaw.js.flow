...
const insertRawBlock = (block: BlockNodeRecord, entityMap: *, rawBlocks: Array<RawDraftContentBlock>, blockCacheRef: *) => {
  ...
  const rawBlock = blockCacheRef[block.getKey()] = { ...createRawBlock(block, entityMap),
    children: []
  };
...
const encodeRawBlocks = (contentState: ContentState, rawState: RawDraftContentState): RawDraftContentState => {
  ...
  contentState.getBlockMap().forEach(block => {
    block.findEntityRanges(character => character.getEntity() !== null, start => {
      const entityKey = block.getEntityAt(start); // Stringify to maintain order of otherwise numeric keys.

      const stringifiedEntityKey = DraftStringKey.stringify(entityKey); // This makes this function resilient to two entities
