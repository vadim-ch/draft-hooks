...
const randomizeContentBlockNodeKeys = (blockMap: BlockMap): BlockMap => {
  const newKeysRef = {}; // we keep track of root blocks in order to update subsequent sibling links
  ...
  return OrderedMap(blockMap.withMutations(blockMapState => {
    blockMapState.forEach((block, index) => {
      ...
      const parentKey = block.getParentKey(); // new key that we will use to build linking

      const key = generateRandomKey(); // we will add it here to re-use it later
