...
const isIE = UserAgent.isBrowser('IE'); // IE does not support the `input` event on contentEditable, so we can't
// observe spellcheck behavior.

const allowSpellCheck = !isIE; // Define a set of handler objects to correspond to each possible `mode`
// of editor behavior.
...
class DraftEditor extends React.Component<DraftEditorProps, State> {
  ...
  _handleEditorContainerRef: (HTMLElement | null) => void = (node: HTMLElement | null): void => {
    this.editorContainer = node; // Instead of having a direct ref on the child, we'll grab it here.
  ...
  _renderARIADescribedBy(): ?string {
    ...
    const placeholderID = this._showPlaceholder() ? this._placeholderAccessibilityID : '';
